...
def BuildCanvas(canvas_height: int, canvas_width: int, node_size: int) -> CanvasData:
    ...
    canvas.pack(pady=20)
    number_of_rows, number_of_columns = CalculateRowsAndColumns(
        canvas_height, canvas_height, node_size
    )
...
def CalculateRowsAndColumns(
    canvas_height: int, canvas_width: int, node_size: int
) -> tuple:
